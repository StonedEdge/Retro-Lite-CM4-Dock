...
void SSD1351_display_text_buffer(const char* text, int textLen, uint32_t color, font_t font)
{
    ...
    while (textLen-- > 0) {
        ...
        if (c == '\n') {                // Need new line -- scroll if required.
            SSD1351_display_text_buffer_advance_cursor_y(fh, color);
        }
        else {
            if (SSD1351_cursor.x == 0 && SSD1351_cursor.y + fh >= OLED_HEIGHT) {
                // We are on a new line. If text isn't going to fit we need to scroll.
                SSD1351_display_text_buffer_advance_cursor_y(fh, color);
            }
            else if (SSD1351_cursor.x + fw >= OLED_WIDTH) {
            ...
            SSD1351_write_char_no_cursor(color, font, c);
            SSD1351_cursor.x += fw;
            SSD1351_update();
        }
    }
...
{
    /* BOGUS (1) Don't use hardcoded sizes, and (2) uints cannot be negative.
...
static void SSD1351_write_char(uint32_t color, font_t font, char c)
{
    ...
    // TODO: Totally questionable in some cases!!

    SSD1351_cursor.x += font.width;

    if ((SSD1351_cursor.x + font.width >= OLED_WIDTH - 1) && (SSD1351_cursor.y + font.height <= OLED_HEIGHT - 1)) {
...
/**
* Write a single character without cursor control.
*
* This is the same as SSD1351_write_char, but it does not do anything with the
* cursor.  This version is used with SSD1351_display_text_buffer, which does
* its own cursor management as well as vertical scrolling.
*
* @param color              [in] A 32-bit fg/bg color.
*
* @param font               [in] A font_t structure.
...
static void SSD1351_write_char_no_cursor(uint32_t color, font_t font, char c) {
    if ((COLUMNS <= SSD1351_cursor.x + font.width) || (ROWS <= SSD1351_cursor.y + font.height)) {
        return;
    }
